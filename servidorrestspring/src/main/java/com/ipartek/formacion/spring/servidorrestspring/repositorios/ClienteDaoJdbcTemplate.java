...
public class ClienteDaoJdbcTemplate implements Dao<Cliente> {
	...
	public Cliente obtenerPorId(Long id) {
		return jdbc.queryForObject("SELECT * FROM clientes WHERE id = ?",
	...
	public Cliente insertar(Cliente cliente) {
		...
		jdbc.update(connection -> {
			PreparedStatement ps = connection.prepareStatement("INSERT INTO clientes (nombre, apellidos) VALUES (?, ?)",
	...
	public Cliente modificar(Cliente cliente) {
		jdbc.update("UPDATE clientes SET nombre = ?, apellidos = ? WHERE id = ?", cliente.getNombre(),
				cliente.getApellidos(), cliente.getId());
		return cliente;
	}
